...
export function Navbar() {
  ...
  const textColor = isHomePage ? "text-white/90 hover:text-white" : "text-slate-500 hover:text-slate-900";
      ...
      <Container>
        <div className="flex items-center justify-between h-20 md:h-28">
          ...
          <div className="hidden md:flex items-center gap-10">
            {NAV_LINKS.map((link) => (
              <a
                key={link.label}
                href={link.href}
                className={`text-[15px] font-semibold transition-colors ${textColor}`}
              >
                {link.label}
              </a>
            ))}
          ...
          <div className="hidden md:block">
            <a href="https://partner.mymooment.com" className="px-6 py-3 rounded-full bg-[#FFDE00] hover:bg-[#F2D200] transition-colors text-slate-900 text-[15px] font-bold flex items-center gap-2 shadow-lg shadow-[#FFDE00]/20">
              Become a Partner
            ...
            <div className="flex flex-col gap-1">
              {NAV_LINKS.map((link) => (
                <a
                  ...
                  className="px-4 py-3 rounded-xl text-sm font-bold text-slate-700 hover:bg-slate-50 hover:text-[#FF0066] transition-colors"
                >
                  {link.label}
                </a>
              ))}
              <div className="mt-3 px-4">
                <a href="https://partner.mymooment.com" className="w-full px-6 py-3 rounded-full bg-[#FFDE00] text-slate-900 text-sm font-bold flex items-center justify-center gap-2">
                  Become a Partner
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M14 5l7 7m0 0l-7 7m7-7H3" />
                  </svg>
